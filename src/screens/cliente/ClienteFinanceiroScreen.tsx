...
export default function ClienteFinanceiroScreen() {
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-violet-50/50 to-white pb-24">
      ...
      <div className="max-w-lg mx-auto px-4 py-6 space-y-3">
        ...
        {!carregando && linhas.length === 0 && !erro && (
          <p className="text-sm text-gray-600">Nenhum pagamento registrado ainda.</p>
        )}
        ...
        <ul className="space-y-2">
          {linhas.map(l => {
            ...
            return (
              <li key={l.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 space-y-1">
                <div className="flex justify-between gap-2 items-start">
                  <div>
                    ...
                    <p className="text-[11px] text-gray-500">{etLabel}</p>
                  </div>
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border shrink-0 ${badge.cls}`}>
                    {badge.txt}
                  </span>
                </div>
                <p className="text-base font-bold text-violet-800">{formatarValor(Number(l.valor_bruto))}</p>
